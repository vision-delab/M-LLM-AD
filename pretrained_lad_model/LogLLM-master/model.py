...
class LogLLM_not_ft(nn.Module):
    ...
    def forward(self, inputs, seq_positions):
        ...
        '''
        print("BERT Input Shape: ", inputs['input_ids'].shape) # (참조)
        # print("BERT Input Content: ", inputs) # (참조)
        batch_size = len(seq_positions) + 1

        outputs = self.Bert_model(**inputs).pooler_output  # dim = 768
        outputs = outputs.float()
        outputs = self.projector(outputs)
        print("BERT Output (Embedding) Shape:", outputs.shape) # (참조)
        # print("BERT Output (Embedding) Content:", outputs) # (참조)
        outputs = outputs.half()
        ...
        print("LLM Input Shape: ", inputs_embeds.shape) # (참조)
        # print("LLM Input Content: ", inputs_embeds) # (참조)
        attention_mask = attention_mask.to(self.device)
        ...
        while not this_peer_finished:
            if len(past_key_values) == 0:
                ...
                ("LLM Output (Logits) Shape:", outputs.logits.shape) # (참조)
                # ("LLM Output (Logits) Content:", outputs.logits) # (참조)
            else:
