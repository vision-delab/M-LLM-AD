...
class CustomDataset(Dataset):
    def __init__(self, file_path, drop_duplicates=False):
        ...
        if drop_duplicates:
            df = df.drop_duplicates(subset='Content', keep='first')
        self.contents = df['Content'].values
        self.sequences = np.array([content.split(' ;-; ') for content in self.contents], dtype=object)
        self.sequences = np.array([content.split(' ;-; ') for content in self.contents], dtype=object)
    ...
    def __getitem__(self, idx):
        return self.sequences[idx], self.labels[idx], self.contents[idx]
...
class CustomCollator:
    ...
    def __call__(self, batch):
        sequences_, int_labels, original_contents = zip(*batch)
        ...
        string_labels = np.array(int_labels).astype(object)
        string_labels[string_labels == 0] = 'normal'
        string_labels[string_labels == 1] = 'anomalous'

        labels_tensor = torch.tensor(int_labels, dtype=torch.long)
