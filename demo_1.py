...
# --- Helper, Image Gen, Inference Functions (변경 없음) ---
# ... (show_pil_image, make_ts_image, make_ts_image_for_testing)
...
def run_inference(input_image_path, model, processor, device):
...
def run_inference_and_eval(input_path, sample_to_run, sample_values, sample_label, args, model, processor, output_dir,
                           device):
...
# --- ✨ (수정됨) 전체 로직을 호출하는 Main 함수 ---

def main():
    """
    스크립트의 전체 실행 흐름을 관리합니다.
    (설정 -> 모델 로드 -> 데이터 로드 -> [이미지 생성] -> 추론/평가)
    """
    ...
    if value_list is not None and label_list is not None:
        sample_to_run = 10  # 테스트할 샘플 번호 지정
        ...
        # --- 4b. 추론 및 평가 실행 ---
        # 반환된 최종 예측 이미지를 'output_image' 변수에 저장
